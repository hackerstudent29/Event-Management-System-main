...
const Pupil = ({ size = 12, maxDistance = 5, pupilColor = "black", forceLookX, forceLookY }) => {
    ...
    return (
        <div ref={pupilRef} className="rounded-full" style={{ width: size, height: size, backgroundColor: pupilColor, transition: 'transform 0.1s ease-out' }} />
    );
};
...
const EyeBall = ({ size = 48, pupilSize = 16, maxDistance = 10, eyeColor = "white", pupilColor = "black", isBlinking = false, forceLookX, forceLookY }) => {
    ...
    return (
        <div ref={eyeRef} className="rounded-full flex items-center justify-center transition-all duration-150" style={{ width: size, height: isBlinking ? '2px' : size, backgroundColor: eyeColor, overflow: 'hidden' }}>
            {!isBlinking && (
                <div ref={pupilRef} className="rounded-full" style={{ width: pupilSize, height: pupilSize, backgroundColor: pupilColor, transition: 'transform 0.1s ease-out' }} />
            )}
        </div>
    );
};
...
export default function Register() {
    ...
    const { register, googleLogin: googleLoginContext } = useAuth();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            showMessage("Signup successful! Please check your email for the OTP.", { type: 'success' });
            navigate("/verify-signup-otp", { state: { email } });
        } catch (err) {
            console.error(err);
            showMessage(err.response?.data?.message || err.message || "Registration failed!", { type: 'error' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const googleLoginAction = useGoogleLogin({
        onSuccess: async (tokenResponse) => {
            setLoading(true);
            const res = await googleLoginContext(tokenResponse.access_token);
            if (res.success) {
                navigate('/');
            } else {
        ...
        onError: () => showMessage('Google Login Failed', { type: 'error' }),
    });

    return (
        <div className="min-h-screen grid lg:grid-cols-2">
            {/* Left Content Section - Animations */}
                ...
                <div className="w-full max-w-[420px]">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        <div className="space-y-2">
                            ...
                            <Input id="name" type="text" placeholder="John Doe" value={name} onChange={(e) => setName(e.target.value)} onFocus={() => setIsTyping(true)} onBlur={() => setIsTyping(false)} required className="h-12" />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="email">Email</Label>
                            <Input id="email" type="email" placeholder="name@example.com" value={email} onChange={(e) => setEmail(e.target.value)} onFocus={() => setIsTyping(true)} onBlur={() => setIsTyping(false)} required className="h-12" />
                        </div>
                        <div className="space-y-2">
                    ...
                    <div className="mt-6 flex flex-col gap-2">
                        <Button variant="outline" className="w-full h-12" onClick={() => googleLoginAction()}>
                            <RiGoogleFill className="me-3 text-[#DB4437] dark:text-white/60" size={16} />
                            Sign up with Google
                        </Button>
                    </div>
