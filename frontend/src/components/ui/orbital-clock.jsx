...
export function OrbitalClock({ date }) {
    // If date is provided, use it. Otherwise use internal timer (current time).
    // However, for "selecting" context, visuals usually reflect the state.
    // The original component had an internal timer. I will support both.
    const [internalTime, setInternalTime] = useState(new Date())
    const [isHovered, setIsHovered] = useState(false)
    const [mousePos, setMousePos] = useState({ x: 0, y: 0 })
    const containerRef = useRef(null)

    const displayTime = date || internalTime;
    ...
    const formatDate = () => {
        return displayTime.toLocaleDateString("en-US", {
            weekday: "short",
            month: "short",
            day: "numeric",
        })
        ...
        >
            ...
            >
                ...
                <div className="absolute inset-2 rounded-full bg-white/95 dark:bg-card border border-slate-200/70 dark:border-border/50 shadow-xl">
                    {/* Inner subtle ring */}
                    <div
                        className={`absolute inset-3 rounded-full border transition-all duration-500 ${isHovered
