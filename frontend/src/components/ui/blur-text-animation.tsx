...
export default function BlurTextAnimation({
    ...
    const textWords = useMemo(() => {
        ...
        return splitWords.map((word, index) => {
            ...
            // Use exponential curve for delay to create a "gathering" effect
            const exponentialDelay = Math.pow(progress, 0.8) * 0.5;

            // Secondary delay to ensure words follow an order
            const baseDelay = index * 0.06;

            // Add subtle randomness to each word's delay
    ...
    useEffect(() => {
        const startAnimation = () => {
            // Small timeout to ensure initial state is caught
            setTimeout(() => {
                setIsAnimating(true);
            }, 200);

            // Calculate max duration needed for the full cycle
            let maxTime = 0;
